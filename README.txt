...
1) Create component for restaurant cards.
...
1) Create file structure for better readability.
2) export default (name of component) to use the component in another file.
3) import (name of the component) from 'directory location'; to import this in another file.
4) export const (variableName) = (variable value) to export multiple objects in single file.
5) import ({variableName,variableName}) from  'directory location'; to import multiple objects.
6) state variable 'useState';
7) const [variableName, set variable function] = useState('intializer');
8) whenever the useState variable updates react re-rendered the browser.
9) React is fast because of its dom operation, it rendered things very fast.
10) Reconciliation algorithm(React fiber);
11) Virtual DOM - it is a representation of actual dom generated by react.
12) React fiber architecture.
13) diff algorithm.
13) React is fast because of its efficient dom manipulation and because of virtual dom.
14) ArrayDestructuring of useState line.
     const [filteredCards, setfilteredCards] = useState(cards);
  also be written as
    - const arr = useState(cards);
    - const filteredCards = arr[0];
    - const setfilteredCards = arr[1];
